...
@ModuleRegister(name = "Jump Circle", category = Category.RENDER, description = "При прыжке из под вас появляется кругляшок.")
public class JumpCircleModule extends Module {
    @Getter private static final JumpCircleModule instance = new JumpCircleModule();

    private final ModeSetting texture = new ModeSetting("Текстура").value("Глоу").values("Тонкий", "Глоу");
    private final ModeSetting outAnimation = new ModeSetting("Анимация").value("None").values("In", "None");
    ...
    private final SliderSetting spawnDur = new SliderSetting("Время появления").value(6f).range(1f, 30f).step(1f);
    private final SliderSetting dyingDur = new SliderSetting("Время смерти").value(4f).range(1f, 30f).step(1f);
    ...
    private String texture() {
        return "circle/" + (texture.is("Тонкий") ? "lean" : "glow_fat");
    }
