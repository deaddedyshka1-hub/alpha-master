...
public class WorldParticles extends ParticlesModule.BaseSettings {
    private final SliderSetting distance = new SliderSetting(prefix + "Дистанция").value(15f).range(5f, 50f).step(1f);
    private final SliderSetting height = new SliderSetting(prefix + "Высота").value(8f).range(5f, 15f).step(1f);
    private final SliderSetting gravity = new SliderSetting(prefix + "Гравитация").value(0.3f).range(0.1f, 1f).step(0.1f);
