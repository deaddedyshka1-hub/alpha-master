...
public class HitParticles extends ParticlesModule.BaseSettings {
    private final ModeSetting physics = new ModeSetting(prefix + "Физика").value("Падать").values("Летают", "Падать");
    private final SliderSetting motion = new SliderSetting(prefix + "Движение").value(0.3f).range(0.1f, 1f).step(0.1f);
